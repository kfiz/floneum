// Forked from https://github.com/huggingface/candle/blob/main/candle-transformers/src/models/modernbert.rs
//! ModernBERT
//!
...
impl RotaryEmbedding {
    fn new(dtype: DType, config: &Config, rope_theta: f64, device: &Device) -> Result<Self> {
        ...
        let t = Tensor::arange(device, max_seq_len as u32, u32)?
            .to_dtype(dtype)?
...
impl ModernBertDecoder {
    fn load(vb: VarBuilder, dev: &Device) -> Result<Self> {
        ...
        let decoder_weights = vb.get("model.embeddings.tok_embeddings.weight", dev)?;
        let decoder_bias = vb.get("decoder.bias", dev)?;
...
impl ModernBertModel {
    pub fn load(device: &Device, vb: &mut VarBuilder, config: &Config) -> Result<Self> {
        let word_embeddings = embedding(device, &mut vb.pp("model.embeddings.tok_embeddings"))?;
...
impl ModernBertForMaskedLM {
    pub fn load(vb: VarBuilder, device: &Device) -> Result<Self> {
        ...
        let decoder = ModernBertDecoder::load(vb.clone(), device)?;
